...
int main(){
    ...
    Adafruit_SSD1306_I2c oled(i2c, LCD_I2C_ADDR << 1);
    ...
    while(true){
        ...
        if(hdc.Temperature < 20){
            //too cold
            LcdTools::print_bitmap(fuckin_cold,&oled);
        }
        else if(hdc.Temperature > 20 && hdc.Temperature < 30){
            //good
        }
        else{
            //too hot
            LcdTools::print_bitmap(fuckin_hot,&oled);
        }
        oled.setTextCursor(10,10);
        oled.setTextColor(WHITE);
        oled.setTextSize(10);
        oled.printf(string(to_string(hdc.Temperature) + "C").c_str());
        wait(1);
    }
    return 0;
}
